...
static PetscErrorCode MatMult_Interp(Mat A, Vec X, Vec Y) {
  ...
  CHKERRQ(ierr);
  ierr = user->VecGetArray(user->Yloc, &y); CHKERRQ(ierr);
  CeedVectorSetArray(user->ceedvecc, user->memtype, CEED_USE_POINTER, x);
  CeedVectorSetArray(user->ceedvecf, user->memtype, CEED_USE_POINTER, y);
...
static PetscErrorCode MatMult_Restrict(Mat A, Vec X, Vec Y) {
  ...
  CHKERRQ(ierr);
  ierr = user->VecGetArray(user->Yloc, &y); CHKERRQ(ierr);
  CeedVectorSetArray(user->ceedvecf, user->memtype, CEED_USE_POINTER, x);
  CeedVectorSetArray(user->ceedvecc, user->memtype, CEED_USE_POINTER, y);
...
static PetscErrorCode ComputeErrorMax(UserO user, CeedOperator op_error,
  ...
  user->VecRestoreArrayRead(user->Xloc, (const PetscScalar **)&x);
  CHKERRQ(ierr);
